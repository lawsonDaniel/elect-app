      ...
      >
        {userInfo && userInfo.map((user: any, index) => (
          <Menu
            ...
            message = {user?.active ? 'active' : `${(Number(new Date()) - Number(new Date(user?.lastseen))) / 60000 < 1 ? 'just now' : (Number(new Date()) - Number(new Date(user?.lastseen))) / 60000 < 60 ? Math.floor((new Date() - new Date(user?.lastseen)) / 60000) + ' minutes ago' : (new Date() - new Date(user?.lastseen)) / 3600000 < 24 ? Math.floor((new Date() - new Date(user?.lastseen)) / 3600000) + ' hours ago' : (new Date() - new Date(user?.lastseen)) / 86400000 < 30 ? Math.floor((new Date() - new Date(user?.lastseen)) / 86400000) + ' days ago' : new Date(user?.lastseen).toLocaleDateString()}`}
            top={0}
